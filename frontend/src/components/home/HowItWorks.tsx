...
export default function HowItWorks() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setImageVisible(true), 30);
    return () => window.clearTimeout(id);
  }, [active]);

  return (
    <section id="how" className="scroll-mt-28 bg-[#f5e7d2] py-[72px] md:py-24">
      <div className="mx-auto max-w-[1240px] px-5 sm:px-8 lg:px-10">
        ...
        <div className="mb-9 grid grid-cols-1 gap-2 sm:mb-11 sm:grid-cols-2 sm:gap-3">
          {TABS.map((entry) => (
            <button
              key={entry.id}
              ...
              className={[
                "min-h-12 w-full rounded-full border px-5 text-[10px] font-bold uppercase tracking-[0.15em] transition-colors",
                active === entry.id
          ...
          <ol className="flex list-none flex-col gap-7 md:gap-8">
            {tab.steps.map((step) => (
              <li key={step.n} className="flex items-start gap-[18px]">
                <span className="w-12 shrink-0 pt-0.5 font-display text-[40px] leading-none tracking-[0.02em] text-[#a67126] sm:text-[44px]">
                  {step.n}
                </span>
                <div>
